...
| `FLOW_ANDROID_CERT_SHA256` | SHA-256 fingerprint(s) of the Android app's signing certificate, colon-separated hex as `keytool -printcert` prints it; comma-separated for several — under Play App Signing that is Google's **app signing** certificate (Play Console → App integrity), plus the debug certificate for dev builds if wanted. Set = the server publishes `/.well-known/assetlinks.json` so Android opens this host's `/join/…` and `/invite/…` links in the app (verified App Links). Unset = a JSON 404 there, which Android reads as "no app links" — the right default until an app is shipped for this host. |
| `FLOW_ANDROID_PACKAGE` | Package name the assetlinks file blesses (default `im.freeflow.app`); only a fork that renamed the app needs it. |
